...
Technical debt: matching GitHub's current `keytar` pattern keeps this pass
small, but `keytar` is archived and native-module-heavy. The implementation
should isolate the gateway credential API so a later migration to Electron
`safeStorage` or another maintained encrypted store changes
`gatewayCredentials.ts` only. If the Azure credential work introduces a shared
`safeStorage` boundary first, prefer reusing that maintained boundary for
gateway credentials instead of adding a second new `keytar` dependency point.

### Secret Handoff

Moving tokens out of SQLite and into the main-process credential boundary is
necessary but not sufficient. Gateway startup must also avoid passing token
values through `spawn` arguments, SSH command strings, shell history, or remote
`ps` output.

Required handoff model:

- Prefer one-shot stdin or an inherited bootstrap pipe for local startup so
  gateway tokens never touch disk. If the runtime cannot support a pipe in the
  first implementation, local startup may write API/admin tokens to permissioned
  files under `app.getPath("userData")/Control/gateways/<safe-area-id>/secrets/`
  using mode `0600` where supported, then start the runtime with token-file
  paths instead of token values.
...
- Startup must also sweep stale files from the local `secrets/` directory before
  every start and during app startup, because Electron can crash or be force
  killed between file creation and cleanup. Stale-file cleanup should remove
  only files with the app-owned naming pattern and restrictive parent directory,
...
2. In packaged macOS apps, resolve executables from a signed executable
   location such as `Control.app/Contents/MacOS/` or `Contents/Helpers/`, not
   `Contents/Resources`, so Gatekeeper, codesign, and notarization treat the
   gateway as executable code. Non-executable metadata may remain under
   resources.
...
- The manifest may live in packaged resources, for example
...
- For local packaged execution, the signed app bundle is the primary integrity
  boundary; a custom SHA-256 check is useful as a diagnostic but redundant with
  platform signing when both manifest and binary are inside the signed bundle.
  Keep SHA-256 verification mandatory for SSH deployment after transport,
  because the remote host receives a copied artifact outside the local bundle's
  execution context.
...
Do not rely on throwing custom `AreaGatewayFailure` objects across Electron IPC.
Electron serialization strips custom error fields and leaves the renderer with a
plain message. Lifecycle IPC routes such as repair, rotate, restart, and stop
should return discriminated results, for example
`{ success: true, summary } | { success: false, failure: AreaGatewayFailure }`.
Operation routes that already return `AreaGatewayOperationResult` should encode
known failures into `status: "failed"` plus the typed code where the shared
...
| stale process id           | persisted PID/processId has no matching live child identity | ignore persisted PID for termination; mark stale metadata | offer repair; never kill by PID alone                                  |
...
- `AreaManager.refreshGatewayArea()` may call direct local discovery for
  ...
  gateway record as `status: "error"` with the original typed gateway failure
  message. The later successful fallback scan must not overwrite the root cause
  with a generic local-discovery success message.
...
Persisted `pid` and SSH tunnel `processId` values are diagnostic metadata, not
authoritative process handles. Stop/retry code may terminate only a live
`ChildProcess` handle owned by the current Electron process, or a PID whose
command, executable path, parent/session identity, and gateway state directory
match the expected gateway process. After app restart or reboot, treat persisted
PIDs as stale and clear them instead of sending a signal that could hit an
...
- Pending migration intentionally locks out gateway startup, verify, stop, and
  credential rotation for affected Areas because the new runtime path is not
  allowed to read fallback SQLite tokens. Renderer UX should show a
  credential-store-unavailable state with cached/direct local data when
  possible, plus a retry action after the OS keychain becomes available.
...
- If token files are used instead of a pipe, startup/app-start cleanup removes
  stale app-owned files from the gateway `secrets/` directory and tests cover a
  crash-before-cleanup simulation.
...
- Persisted PIDs/process IDs are never used for termination unless current
  process identity is verified; stale persisted IDs are cleared instead of
  signaled after restart.
...
- Local Areas may show stale or direct local repository data after gateway
  ...
  records the failure.
- Local fallback preserves the original gateway failure message even when direct
  local discovery succeeds afterward.
